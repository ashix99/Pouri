...
- مرحله اول بدون نیاز به `RATE_DAY`
- مرحله دوم با جدول کامل در صورت وجود `RATE_DAY`
- نگه‌داشتن لیست معلق کاربر تا وقتی که بعدا فقط `RATE_DAY` را بفرستد
...
برای فونت PDF، ربات اول `Yekan Bakh` را از این مسیرها امتحان می‌کند:
...
اگر فونت یکان‌بخ موجود نباشد، ربات موقتاً از فونت fallback سیستم استفاده می‌کند.
...
اگر `RATE_DAY` را نگذارید، ربات مرحله اول را می‌فرستد و بعد از شما نرخ روز می‌خواهد.
